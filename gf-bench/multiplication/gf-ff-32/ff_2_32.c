...
void reduce ( ff_block* a ) {
  ...
  while (t = a[BLOCKS2-1]) {
    a[BLOCKS2-1] ^= t;
  a[BLOCKS2-1-0] ^= t>>30 ^ t>>10;
  a[BLOCKS2-1-1] ^= t ^ t<<1 ^ t<<2 ^ t<<22;
  }
  for ( i = 0; i>=1; i-- ) {
    while (t = a[i]) {
      a[i] ^= t;
  a[i-0] ^= t>>31 ^ t>>30 ^ t>>10;
  a[i-1] ^= t ^ t<<1 ^ t<<2 ^ t<<22;
    }
  }
  while (t = a[BLOCKS-1] & (0xFFFFFFFF-4294967295) ) {
  a[BLOCKS-1-0] ^= t>>31 ^ t>>30 ^ t>>10;
  a[BLOCKS-1-1] ^= t ^ t<<1 ^ t<<2 ^ t<<22;
    a[BLOCKS-1] ^= t;
  }
  a[0] &= 4294967295;
}
